...
public struct Creature: Equatable, Hashable, Sendable {
    ...
    /// External sources for the creature.
    public var externalSources: [Entry] = []
    ...
    ///
    /// For convenience, ``alignment`` may be specified as an array of ``Alignment``, which provides static properties
    /// for common sets.
    /// ```swift
    /// creature.alignment = [.lawfulGood]
    /// ```
    ///
    /// ## Formatting
    /// ``Alignments/FormatStyle`` formats ``alignment`` in the representation used in stat blocks:
    /// ```swift
    /// Creature.Alignments([.lawfulGood, .chaoticGood])
    ///     .formatted()
    /// // "Lawful Good or Chaotic Good"
    /// ```
    public var alignment: Alignments = []
    ...
    ///
    /// Usually provided as a ``DiceNotation`` formula given in the associated value of ``Resource/resource(_:value:)``.
    /// The `value` associated value is `nil` if the initial value matches the ``DiceNotation/average``, otherwise
    /// provides the initial value.
    ///
    /// In the event the formula is not parseable, or only a value is given, the value and formula are given in the
    /// associated values of ``Resource/value(_:formula:)``.
    ///
    /// For convenience ``Resource`` values may be specified as integer and string literals:
    /// ```swift
    /// creature.resources = ["Souls": "2d4", "Mana": 8]
    /// ```
    ///
    /// ## Formatting
    /// ``Resources/FormatStyle`` formats ``resources`` in the representation used in stat blocks:
    /// ```swift
    /// Resources(["Souls": "2d4"])
    ///     .formatted()
    /// // "Souls 5 (2d4)"
    ...
    // TODO: brew externalSources - array of {entry: Entry}
    ...
    // Href {
    //   case local(name: String, source: String)
    //   case path(String)
    //   case url(URL)
    // }

    // token: {name, source} pair
    // tokenUrl: URL
    // tokenHref -> mediaHref -> {internal: path} or {external: string}
    // tokenHref3d -> as tokenHref
    // foundryTokenSubjectHref -> as tokenHref
    // foundryTokenSubjectScale: Float

    // foundryImg: String
    // foundryAdvice: Entry
    // foundryPrototypeToken: {...}

    // altArt: site (name, source, page, credit, custom, tags)
    //      or url (URL + site + foundryScale)
    //      or token or href
    ...
    /// Whether token image is custom or otherwise unofficial.
