...
public extension LocalizedStringKey.StringInterpolation {
    ...
    mutating func appendInterpolation(_ resource: Resource) {
        appendInterpolation(resource, format: .resource.attributed)
    }

    mutating func appendInterpolation(_ resource: Resource, format: Resource.FormatStyle) {
        appendInterpolation(resource, format: format.attributed)
    }
    ...
    ScrollView {
        VStack(alignment: .leading) {
            ...
            Text("Resource")
                .font(.headline)
                .padding(.top, 8)
            Text("\(Resource.resource("4d6 + 10"))")
            Text("\(Resource.value(2, formula: "coin toss"))")
