...
public extension LocalizedStringKey.StringInterpolation {
    ...
    mutating func appendInterpolation(_ alignments: Creature.Alignments) {
        appendInterpolation(alignments, format: .alignments().attributed)
    }

    mutating func appendInterpolation(_ alignments: Creature.Alignments,
                                      format: Creature.Alignments.FormatStyle)
    {
        appendInterpolation(alignments, format: format.attributed)
    }
    ...
    mutating func appendInterpolation(_ resources: Creature.Resources) {
        appendInterpolation(resources, format: .resources.attributed)
    }

    mutating func appendInterpolation(_ resources: Creature.Resources, format: Creature.Resources.FormatStyle) {
        appendInterpolation(resources, format: format.attributed)
    }
    ...
    ScrollView {
        VStack(alignment: .leading) {
            Text("Alignment")
                .font(.headline)
                .padding(.top, 8)
            Text("\(Creature.Alignment.chaoticGood)")
            Text("\(Creature.Alignment.chaoticGood, format: .alignment(case: .lowercased))")

            let alignmentChance = Creature.Alignment.alignment(.lawfulEvil, chance: 75)
            Text("\(alignmentChance)")
            Text("\(alignmentChance, format: .alignment(case: .lowercased))")

            let alignmentNote = Creature.Alignment.alignment(.chaoticEvil, note: "when fed after midnight")
            Text("\(alignmentNote)")
            ...
            Text("Alignments")
                .font(.headline)
                .padding(.top, 8)
            let alignments = Creature.Alignments(.anyChaotic)
            Text("\(alignments)")
            Text("\(alignments, format: .alignments(case: .lowercased))")

            let alignmentChoice = Creature.Alignments([.lawfulGood, .lawfulEvil], prefix: "Typically ")
            Text("\(alignmentChoice)")
            Text("\(alignmentChoice, format: .alignments(case: .lowercased))")
