    ...
    func `Adventure with standard fields`() throws {
        try testCodable(
            ...
            value: Adventure(name: "Essentials Kit: Dragon of Icespire Peak",
                             ...
                             contents: [
                                 Contents("Running the Adventure",
                                          headers: Header("Overview"),
                                          Header("The Forgotten Realms"),
                                          Header("First Session: Character Creation"),
...
struct AdventureGroupCodableTests {
    ...
    func `adventure groups`(adventureGroup: Adventure.Group) throws {
        try testCodable(
            json: """
...
struct AdventureLevelCodableTests {
    ...
    func `Level with custom`() throws {
        try testCodable(
            ...
            """,
            value: Adventure.Level.special("Players use monster stat blocks"),
        )
    }
}
...
struct AdventureLevelStringTests {
    ...
    func `description for range`() {
        let level = Adventure.Level.range(3...15)
        #expect(String(describing: level) == "3–15")
    }

    @Test
    func `description for special`() {
        let level = Adventure.Level.special("Players use monster stat blocks")
        #expect(String(describing: level) == "Players use monster stat blocks")
    }
}
