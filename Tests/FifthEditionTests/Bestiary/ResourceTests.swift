...
struct ResourceComparableTests {
    @Test
    func `resource(:_) compare by average`() {
        #expect(Resource.resource(DiceNotation(.d6, count: 4, modifier: 10))
            < Resource.resource(DiceNotation(.d8, count: 4, modifier: 10)))
    }

    @Test
    func `resource(:_) compare by average uses value`() {
        #expect(Resource.resource(DiceNotation(.d6, count: 4, modifier: 10), value: 14)
            < Resource.resource(DiceNotation(.d6, count: 4, modifier: 10)))
    }

    @Test
    func `value(:_) compare by average`() {
        #expect(Resource.value(14, formula: "coin toss") < Resource.value(15, formula: "coin toss"))
    }

    @Test
    func `resource(:_) compare to value(:_) by average`() {
        #expect(Resource.resource(DiceNotation(.d6, count: 4, modifier: 10))
            < Resource.value(25, formula: "coin toss"))
    }

    @Test
    func `resource(:_) compare to value(:_) by given value`() {
        #expect(Resource.resource(DiceNotation(.d6, count: 4, modifier: 10), value: 14)
            < Resource.value(15, formula: "coin toss"))
    }

    @Test
    func `value(:_) compare to resource(:_) by average`() {
        #expect(Resource.value(20, formula: "coin toss") <
            Resource.resource(DiceNotation(.d6, count: 4, modifier: 10)))
    }

    @Test
    func `value(:_) compare to resource(:_) by given value`() {
        #expect(Resource.value(10, formula: "coin toss") <
            Resource.resource(DiceNotation(.d6, count: 4, modifier: 10), value: 14))
    }
}

struct ResourceFormatStyleTests {
    static let locale = Locale(identifier: "en_US")

    @Test
    func `resource() formats notation and average value`() throws {
        let notation = DiceNotation(.d12, count: 19, modifier: 133)
        let resource = Resource.resource(notation)

        let formatter = Resource.FormatStyle().locale(Self.locale)
        let description = formatter.format(resource)
        #expect(description == "256 (19d12 + 133)")

        let attributed = formatter.attributed.format(resource)
        #expect(String(attributed.characters) == description)
        #expect(attributed.resource == resource)
    ...
    @Test
    func `resource() formats notation and given value`() throws {
        let notation = DiceNotation(.d12, count: 19, modifier: 133)
        let resource = Resource.resource(notation, value: 128)
    ...
    @Test
    func `resource() formats value with formula`() {
        let resource = Resource.value(2, formula: "coin toss")
...
struct ResourceInitTests {
    @Test
    func `init(integerLiteral:) sets value`() {
        let resource: Resource = 10
        #expect(resource == .value(10))
    }

    @Test
    func `init(stringLiteral:) sets resource`() {
        let resource: Resource = "4d6 + 10"
        #expect(resource == .resource(DiceNotation(.d6, count: 4, modifier: 10)))
    }
}
